...
for (_, img_pair, _, drive, idx_sample) in kitti_dataset:
    ...
    save_image(depth_single_img / depth_single_img.max(),
               depth_folder + '/' + str(idx_sample+1).zfill(10) + '.png')
    torch.save(depth_single_img, depth_folder + '/' +
               str(idx_sample+1).zfill(10) + '.pt')

    print(drive + "_" + str(idx_sample+1).zfill(10))
